...
4. 🆕 确认年龄页面 (auth.openai.com/about-you)
   - 输入姓名
   - 选择生日（随机 20-25 岁）
5. 登录成功 → 自动跳转 SheerID 验证页面
6. 🆕 选择 Status："Military Veteran or Retiree"
7. 填写表单（Branch、姓名、生日、退伍日期、邮箱）
8. 点击 "Verify My Eligibility"
9. **收 SheerID 验证邮件**（来自 verify@sheerid.com，包含链接！）
10. 点击链接完成验证
11. 成功 → 跳转 chatgpt.com（账号获得1年Plus）
```
...
⚠️ **必须使用固定 Chrome 实例**（保留代理插件等配置）

```bash
# 启动脚本位置
scripts\start-chrome-devtools.bat

# 使用默认 Chrome 配置（保留代理插件）
set "USER_DATA_DIR=%USERPROFILE%\AppData\Local\Google\Chrome\User Data"
```

**启动步骤**：
1. 关闭所有 Chrome 窗口
2. 运行 `scripts\start-chrome-devtools.bat`
3. Chrome 启动后带有代理插件
